...
__global__ void __launch_bounds__(1024, 2)
    ...
    volatile __shared__ unsigned int s_out[warpsPerBlock];
    {
        ...
        if (threadIdx.y == 0 && threadIdx.x < 2) { // 2 threads, 2 coordinates
            register float k;
            k = g_K[blockIdx.x*4 + threadIdx.x];
            if (k < _warpSize || (threadIdx.x == 0 && imgWidth-_warpSize < k) || (threadIdx.x == 1 && imgHeight-_warpSize < k)) {
        ...
        if (threadIdx.y == 1 && threadIdx.x < 2) {
            register float s = g_K[blockIdx.x*4 + 2 + threadIdx.x]; // Just a temp variable... thread 0 has scale, 1 has angle.
            register float u, v; // cos and sin of (1,0) rotated by angle
            if (threadIdx.x == 1) {
                if (s == -1.0f) {
                    u = 1.0f;
                    v = 0.0f;
...
void loadPatchTriplets(cudaArray* cuArray) {
    ...
    patchTriplets.addressMode[0] = cudaAddressModeClamp;
    patchTriplets.addressMode[1] = cudaAddressModeClamp;
